...
startGame = () => {
  ...
  document.addEventListener("keyup", (e) => {
    ...
    if ("KeyA" <= e.code && e.code <= "KeyZ") {
      // nice way of getting all letters from A-Z
      if (column < columns) {
        let currentTile = document.getElementById(
          row.toString() + "-" + column.toString()
        );
        if (currentTile.innerText == "") {
          currentTile.innerText = e.code[3]; //[#] way of just getting the letter as it was giving me "KEYQ" rather than just the letter
          column += 1; //go to next column to continue entering next letter
        }
      }
...
function update() {
  ...
  let map = {};
  //learned making an "empty object" (map) allows each letter to get sorted/counted which helps make sure its displaying correct count to player and not duplicates. Don't want them seeing more yellow or green tiles than there are.
  ...
  //Check for right letter on correct tile and if won.
  for (let i = 0; i < columns; i++) {
    let currentTile = document.getElementById(
  ...
  //Check for all other scenarios (letter not in word and right letter on wrong tile)
  for (let i = 0; i < columns; i++) {
    let currentTile = document.getElementById(
      row.toString() + "-" + i.toString()
    );
    let letter = currentTile.innerText;
    if (!currentTile.classList.contains("green"))
      if (word.includes(letter) && map[letter] > 0) {
        currentTile.classList.add("yellow");
        map[letter] -= 1;
      } else {
        currentTile.classList.add("darkgray");
      }
  }
}